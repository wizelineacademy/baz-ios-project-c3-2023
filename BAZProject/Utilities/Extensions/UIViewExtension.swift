...
public extension UIView {
    ...
    static func fillSkeletons(onView aView: UIView) {
        if (aView as? UILabel) != nil || (aView as? UIButton) != nil || (aView as? UIImageView) != nil {
            aView.removeSkeletonAnimation()
            aView.addSkeletonAnimation()
        }
        aView.subviews.forEach { theSubView in
            UIView.fillSkeletons(onView: theSubView)
        }
    }
    
    static func removeSkeletons(onView aView: UIView) {
        aView.removeSkeletonAnimation()
        aView.subviews.forEach { theSubView in
            UIView.removeSkeletons(onView: theSubView)
        }
    }
}
